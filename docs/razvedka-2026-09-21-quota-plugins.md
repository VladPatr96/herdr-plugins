...
- **Где лежит ключ API DeepSeek у пользователя** — по учётным файлам не искал намеренно: секреты не читаю без спроса. Нужен ответ человека: переменная окружения, файл, менеджер паролей.
